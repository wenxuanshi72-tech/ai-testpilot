...
def test_existing_0003_database_upgrades_through_phase6(
    ...
    database.migrate()
    assert database.fetch_one("SELECT COUNT(*) AS count FROM schema_migrations") == {"count": 11}
    ...
    assert {
        "canonical_bug_records",
        "canonical_bug_sources",
    ...
    assert {
        "evidence_consolidation_runs",
        "consolidated_evidence_records",
        "deterministic_failure_classifications",
        "advisory_ai_analyses",
